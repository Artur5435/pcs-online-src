...
export class OlejeEkoComponent implements OnInit {
  ...
  constructor(private oilsService: OilsService, private itemService: ItemService) {
    this.oilsService.getOilsEko().subscribe((data) => { this.items = data; });
    this.StartSort();
  }
  ngOnInit() {
    $("#Title").text("OLEJE - KLASA EKONOMICZNA");
    this.items.push({
      Producent: "VALVOLINE",
      ID: "VAC10W404.1",
      TypProduktu: "OLEJ",
  ...
  AddToCart(id) {
    ...
    let itemLocal: Item = this.items[id];
  ...
  AddToCartPop(id, count) {
    //var item = this.oils[id];
    let itemLocal: Item = this.items[id];
    this.itemService.AddToCartPop(itemLocal, count);
  }
